...
class ChessboardRecognition:
        ...
        while(True):
            ...
            # if the user presses s then run corner detecion on the image
                ...
                if self.pixel_coordinates is None:
                    ...
                    print(len(self.pixel_coordinates))
    ...
    def calibrate_camera(self):
        ...
        for i in range(len(chessboard_3d_corners_for_calibration)):
            img_points, _ = cv2.projectPoints(chessboard_3d_corners_for_calibration[i], self.rotation_vector[i], self.translation_vector[i], self.camera_matrix, self.distortion_coeffcients)
    ...
    def _pre_process_img(self):
        ...
        # undisort the image using the intrinsic and distortion coefficients
        if self.intrinsic_camera_matrix is not None and self.distortion_coeffcients is not None:
            self.processing_img = cv2.undistort(self.processing_img, self.intrinsic_camera_matrix, self.distortion_coeffcients, None, self.intrinsic_camera_matrix)
    ...
    def crop_and_get_top_down_view_of_chessboard(self, corners):
        ...
        # 
        destination_points = np.array([[0, 0], [WIDTH, 0], [WIDTH, HEIGHT-PADDING], [0, HEIGHT]], dtype ='float32')
        # get the transformation matrix for a top down view of the original image

        # destination_points = np.array([[PADDING, PADDING], [WIDTH - PADDING, PADDING], [WIDTH - PADDING, HEIGHT - PADDING], [PADDING, HEIGHT - PADDING]], dtype ='float32')
...
def test_chessboard_recognition(chessboard_recogniser, frame):
    ...
    if not chessboard_recogniser.get_pixel_coordinates_using_shi_tomasi_on_warped_img():
        pass  #  if you cant 81 corners of the img        
